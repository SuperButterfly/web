...
const States = () => {
  ...
  const [statesList, setStatesList] = useState([
    'focus', 'hover', 'active', 'disabled', 'focus-within', 'focus-visible'
  ]);
  ...
  const discardState = (name) => {
    let aux = statesSelected.filter(state => state.name != name)
    setStatesSelected(aux);
  ...
  const addProperty = stateName => {
    const aux = statesSelected.map(state => {
      if (state.name === stateName) {
        let temp = state.properties
        let last = state.properties.length - 1
  ...
  return (
    <div className="states-container">
      <div 
        className={haveStates || isOpen ? "states-container1" : "states-container1 closed"}
      >
        ...
        <div 
          ...
          onClick={addState}
          style={ isOpen && statesList.length > 0 ? { display: "flex" } : { display: "none" }}
        >
          <span
            className='states-close'
            onClick={() => setIsOpen(false)}
          >
            x
          </span>
          {
            statesList && statesList.length > 0 && statesList.map((state, idx)=>(
              <span 
                className="states-text" 
                id={`${state}/${idx}`} 
                key={idx}
              >
                {state}
      ...
      <div className="states-container3">
      {
        ...
        statesSelected.map(({name}, idx) => (
          <div className="states-container4" key={idx}>
            ...
            <div className="states-container5" onClick={() => discardState(name)}>
          ...
          statesSelected && statesSelected.length > 0 ?
            statesSelected.map(({name, activated, properties}, idx) =>  
              activated && properties && properties.length > 0 ? 
                properties.map(({ isCheked, propertyName, valueName}, idx) => (
                  <Property 
                    key={idx}
                    stateId={`${name}/${idx}`}
                    isCheked={isCheked}
