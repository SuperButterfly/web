...
const Clases = () => {
  ...
  const handleInputChange = ev => {
    ...
    if (ev.target.value === '') {
      ...
      const found = classesList.filter(item => item.name.includes(ev.target.value));
      if (found) {
  ...
  const handleCheck = ev => {
    let aux = classesSelected.map(item => {
  ...
  const releaseClass = ev => {
    const aux = classesSelected.filter(classSelected => classSelected.name === ev.target.id)
    console.log(aux)
    setClassesSelected(aux);
  }
  ...
  return (
    <div className="clases-container">
      ...
      {
        menuOpen1 && <div className="clases-container6">
          {
            menuOpen2 ?
              <div className="clases-container10" id={'inputValue'} onClick={handleclassesList}>
                <span id={inputValue}>Create a class</span>
                <span className="clases-text1" id={'inputValue'}>{inputValue}</span>
              </div>
            : null
          }
          {
            ...
            classesSelected.map(({name, isChecked}, idx) => (
              <div className="clases-container2" key={idx}>
                ...
                <div className="clases-container4">
                  ...
                  >
                    <path id={idx} d="M213.333 554.667h597.333c23.552 0 42.667-19.115 42.667-42.667s-19.115-42.667-42.667-42.667h-597.333c-23.552 0-42.667 19.115-42.667 42.667s19.115 42.667 42.667 42.667z"></path>
                  </svg>
                </div>
              </div>
            )
