...
const FontsSettings = () => {
  ...
  const handleFontChange = (ev) => {
    ...
    axios
      .get(
        'https://www.googleapis.com/webfonts/v1/webfonts?key=AIzaSyArNHqjUxu6uV7L5UBZHJVgi9ru83eUaKY'
      )
      .then((response) => {
  ...
  const handleSubmit = (ev) => {
    ...
    if (fontFile || data.url) {
      ...
      const fontNameValue =
        fontName !== '' ? fontName : fontFile?.name || data.url

      // Obtener el objeto de la fuente seleccionada del JSON
      const selectedOptionObject = jsonData.find(
      ...
      <div className="settings-general-container01">
        ...
        <div className="fonts-settings-url-container">
          ...
          <input
            type="text"
            id="url"
            className="fonts-settings-url"
            placeholder="copy and paste a font url"
            value={data.url}
          ...
          />
          <span className="settings-general-name-text">
            Or you can pick one of our available fonts.
          </span>
          <select
        ...
        {showError && (
          <div className="error-line">
            {' '}
            <span>some fields are empty</span>{' '}
          </div>
        )}
