...
const possibleTagChanges = [
	["div","article","section","aside","nav","header","main","footer","figure"],
	["span","label","strong","p","h1","h2","h3","h4","h5","h6","b","i","em","u","s","del","pre","code","blockquote","figcaption","cite"],
	["ul,ol,dl"],
	["li","dt","dd"],
]
...
const Attributes = () => {
 ...
 const [availableAttributes, setAvailableAttributes] = useState({})
  ...
  const handleAttributes = ev =>{
    const actAttributes = {...componentSelected.attributes, [ev.target.name]:ev.target.value};
  ...
  const options = possibleTagChanges.filter(tags => tags.includes(componentSelected.tag))[0]
  
  return (
    <div className="attributes-container">
        <div className="attributes-component-header">
          <span className="attributes-title">Attributes</span>
          <svg viewBox="0 0 1024 1024" className="attributes-arrow"
        ...
        <div className="attributes-container-main" style={isOpen ? { display: 'flex'} : { display: 'none' }} >
          <div className="attributes-container2">
            ...
            <span className="attributes-tag-name">{componentSelected.tag}</span>
            {
              ...
              options && options.length && options.length>0 && <div 
            ...
            }
            {
              isListOpen && 
                <div className="attributes-container6">
                  {
                    options && options.length && options.length>0?options.map(option => <div
                      key={option}
                      ...
                      id={option}
                    >
                      <span key={option}  onClick={()=> setListOpen(false)}>{option}</span>
                    </div>) : null
                  }
                </div>
            }
          ...
          {
            availableAttributes&&Object.keys(availableAttributes).map(att=>
              att!=='id'?<div key={att} className="attributes-container4">
                ...
                />
                {
                  componentSelected.nativeAttributes&&!componentSelected.nativeAttributes.includes(att)&&<svg 
          ...
          <div className="attributes-container5">
            ...
            <input name='custom_attr' 
             ...
             <svg viewBox="0 0 1024 1024" 
              className="attributes-icon" 
              onClick={()=> input.custom_attr_tag.length && input.custom_attr.length ? addAttributes(input) : null}>
