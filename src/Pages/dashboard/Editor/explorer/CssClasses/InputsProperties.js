...
export function InputsProperties({ propertyAddList, propertyKeys, objProperties, inputProperty, showFourthList, showFifthList, showList, handleAddProperty, handleAddValue, handleDeleteProperties, classnameChange }) {
  ...
  useEffect(() => {
    if (propertyAddList && typeof propertyAddList === 'object') {
      let result = Object.entries(propertyAddList).map(([key, value]) => ((key && value) && { key, value }));
  ...
  const handleDeleteProperty = (index) => {
    const filteredResult = propertyObjsList.filter(item => item != propertyObjsList[index])
  ...
  const handleBlurAddDeleteProperty = (e, index)=>{
     ...
     if(e.target.value == ""){
      const filteredResultList = propertyObjsList.filter(item=> item != propertyObjsList[index])
    ...
    <>
      {propertyObjsList.map((item, index)=>{
                 ...
                 <div className="left-property" key={index}>
                      {item.key && item.value && <label className="pt-ckbox">
                       <input
                          name={index}
                          type="checkbox"
                          id={index}
                      ...
                      <div className="jsx-108338622">
                        ...
                        <div className=".pt-input-adaptive">
                          ...
                          {item.key && item.value && <div className="input-addon-wrapper">
                            <input
                              type="text"
                              className={`jsx-2523288086 ${checked[index] ? null : "text-decoration"}`}
                            ...
                            ></input>
                            ;
                           {((showList.indexMenu == index) && showList.fourth) && <div className="collapsable-container-fourth">
                               {propertyKeys.map((item, index)=>{
                                   return(
                                        <div className="collapsable-container-list" key={index} onClick={()=>handleAddPropertyList(index, item)}>
                            ...
                            {((showList.indexMenu == index) && showList.fifth) && <div className="collapsable-container-fifth">
                               {objProperties[inputProperty || 'align-content']?.map((item, index)=>{
                                   return(
                                        <div className="collapsable-container-list" key={index} onClick={()=>handleAddValueList(item)}>
                                            <span className="collapsable-item">{item}</span>
                                        </div>  
                                         )
                                       })
                    ...
                    {(item.key && item.value) && <button className="input-delete-button" onClick={()=>handleDeleteProperty(index)}>
                      <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 256 256"><path fill="currentColor" d="M205.66 194.34a8 8 0 0 1-11.32 11.32L128 139.31l-66.34 66.35a8 8 0 0 1-11.32-11.32L116.69 128L50.34 61.66a8 8 0 0 1 11.32-11.32L128 116.69l66.34-66.35a8 8 0 0 1 11.32 11.32L139.31 128Z"/></svg>
                      </button>}
                    </div> 
