...
const ContextStates = ({visible,setVisible,posicion,setStateSelected}) =>{
  ...
  const statesPossibles = ["focus","hover","active","focus-within","focus-visible"]
  ...
  const handleClick = stateName =>{
    setVisible(false)
    let newState = {...statesComponent,[stateName]:{}}
  ...
  return(
    <div className="modal-container" style={ visible ? {
      ...
      <div className="section-option">
        { statesPossibles.map(
            state=>!statesComponent[state]?<div key={state} className="container-option" onClick={()=>handleClick(state)}>
            <span>{state}</span>
          </div>:null)
        }
      </div>
