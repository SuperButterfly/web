...
const DropdownPopup = React.forwardRef(({ props }, ref) => {
  const { sheet, datatable, updateFromDropdown } = props
  const hasLabelsCreated =
    datatable[sheet?.selectedCell[0]]?.[sheet?.selectedCell[1]]?.columnLabels
      ?.length ?? 0
  const labelsCreated =
    datatable[sheet?.selectedCell[0]]?.[sheet?.selectedCell[1]]?.columnLabels ??
    []
  ...
  function handleDelete(labelName, column) {
    const database = JSON.parse(JSON.stringify(datatable))
    database.forEach((row, rowIndex) => {
      row[column].columnLabels = row[column].columnLabels.filter(
  ...
  useEffect(() => {
    hasLabelsCreated === 0
      ? setPlaceholder('Create Label')
      : setPlaceholder('Create or find Label')
  }, [hasLabelsCreated])
    ...
    <div ref={ref} id="DropdownPopup" className={styles.container}>
      <section className={styles.contents}>
        ...
        {input !== '' && (
          <button
            className={
              labelsCreated.some((label) => label === input.trimStart()) ||
              input.trimStart().length === 0
                ? styles.buttonDisabled
                : styles.addButton
            }
            type="button"
            onClick={(event) => handleAddButton(event, sheet?.selectedCell[1])}
            disabled={
              labelsCreated.some((label) => label === input.trimStart()) ||
              input.trimStart().length === 0
            }
