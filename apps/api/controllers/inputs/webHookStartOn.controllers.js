...
async function createWatcher(req, res, next) {
  ...
  const { name, description, address, network, type, webhookUrl, confirmationsBlocks } = req.body;
...
async function createWatcherOnManyNetworks(req, res, next) {
  const url = '/v3/watcher/';
  const { networks, name, description, address, type, webhookUrl, confirmationsBlocks } = req.body;
...
async function updateWatcher(req, res, next) {
  const { id } = req.params;
  const url = `/v3/watcher/`;
  const { paused, webhookUrl, confirmationsBlocks, name, description } = req.body;
