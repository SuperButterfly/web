...
const replaceWord = (data, word) => {
  if (word === 'rootClassName') {
    const temp = data.replace(' ${props.rootClassName} ', '')
    const p1 = temp.search('className={`') + 12
    let p2 = p1
    while (temp[p2] !== '`') {
      p2++
    }
    const className = temp.slice(p1, p2)
    return data.replace(
      '{`' + className + ' ${props.rootClassName} `}',
      `"${className}"`
    )
  } else {
    const p1 = data.search(word + ": '") + word.length + 3
    let p2 = p1
    while (data[p2] !== "'") {
      p2++
    }
    const found = data.slice(p1, p2).replace(/\\n/g, ' ')

    if (
      word.includes('mage_src') ||
      word.includes('mage_alt') ||
      word.includes('_placeholder')
    ) {
...
const quitDanger = (filterString) => {
  const idx1 = filterString.indexOf(danger)
  let first = filterString.slice(0, idx1 - 1)
  let last = filterString.slice(idx1 + 25)
  const idx2 = first.lastIndexOf('<') - 1
  const idx3 = last.indexOf('>') + 1
