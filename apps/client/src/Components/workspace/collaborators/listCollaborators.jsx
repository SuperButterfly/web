...
import { shareRemove, shareUpdate } from '@/redux/actions/workspaces'
            ...
            <tr style={{ position: 'relative' }} key={index}>
              <td className="table-collaborators-avatar">
                <img
                  src={user.avatar}
                  onError={(e) => {
                    e.target.src = '/workspace/assets/user.svg'
                  }}
                />
              </td>
              <td>{user.username}</td>
                ...
                <div
                  className={
                    userLogged.id == user.id
                      ? 'collaborators-roles-opacity collaborators-roles'
                      : 'collaborators-roles'
                  }
                >
                  {/* <div
              ...
              {userLogged.id != user.id && (
                <td className="menuworkspacedots-container">
                  <span
                    onClick={() => showDeleteList(index)}
                    className="menuworkspace-dots"
                  >
                    ...
                  </span>
                  {showDelete && index == menuDeleteIndex && (
                    <div className="collaborators-container19">
                      <span
