...
const Visibility = () => {
  ...
  useEffect(()=>{
    if(componentSelected.properties&&componentSelected.properties.style){
      setOpen((componentSelected.properties.style.display&&componentSelected.properties.style.display==="none")||(componentSelected.properties.style.opacity))
      setInput({opacity:componentSelected.properties.style.opacity?componentSelected.properties.style.opacity*100:"" })
    ...
    <div className="visibility-container">
      <div className="visibility-container1">
        ...
        <div className="visibility-container02" style={{ display: isOpen? "flex" : "none"}}>
          <svg onClick={handleDisplay} className="visibility-icon" fill={isVisible?"#14A9FF":"#565656"} width="20" height="14" viewBox="0 0 20 14" xmlns="http://www.w3.org/2000/svg">
            <path fill-rule="evenodd" clip-rule="evenodd" d="M4.24 5.523C5.77 6.875 7.84 8.06 10.085 8.06c2.274 0 4.35-.96 5.862-2.173.755-.606 1.358-1.267 1.768-1.886.419-.63.608-1.172.608-1.547h.909c0 .634-.297 1.355-.759 2.05-.469.707-1.139 1.435-1.957 2.092-1.634 1.311-3.904 2.373-6.43 2.373-2.557 0-4.834-1.34-6.448-2.766a12.95 12.95 0 01-1.932-2.112 6.576 6.576 0 01-.544-.886c-.12-.244-.224-.514-.224-.751h.91c0 .004.003.033.023.095.02.066.055.15.106.254.102.207.259.466.469.76.42.586 1.032 1.287 1.794 1.959z"></path><path fill-rule="evenodd" clip-rule="evenodd" d="M9.588 11.546V8.817h.91v2.729h-.91zM18.763 8.299l-2.39-2.5.657-.629 2.39 2.5-.657.629zM.58 7.545L2.852 5.17l.657.629-2.27 2.374-.657-.628zM15.096 10.786l-1.69-3.062.796-.439 1.69 3.062-.796.44zM3.892 10.193l1.605-2.908.796.44-1.605 2.908-.796-.44z"></path>
          </svg>
          <input name="opacity" pattern="^-?\d*\.?\d+$" className="input-visibility" value={input.opacity} onChange={handleInputChange} onBlur={handleBlur} placeholder="100%"/>
        </div>
      </div>
    </div>
  )
