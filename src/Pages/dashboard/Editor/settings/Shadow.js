...
const Shadow = () =>{
  ...
  useEffect(()=>{
    if(componentSelected.properties&&componentSelected.properties.style){
      const shadowComponent = componentSelected.properties.style.boxShadow? componentSelected.properties.style.boxShadow : "0 0 0 0 #000000";
      ...
      shadowComponent.split(",").forEach((prop,pos)=>{
        ...
        for(const key in shadowComponent){
          shadowComponent[key] = shadowComponent[key] && shadowComponent[key].slice(-2) === "px"? shadowComponent[key].substring(0, shadowComponent[key].length-2):shadowComponent[key]
        }
        sectionsShadow={
          ...sectionsShadow,
          [pos]:shadowComponent
  ...
  const handleDispachUpdateComponent = stateShadow =>{
    let arrBoxShadow = Object.keys(stateShadow).map(inp=>{
      let arrProps = Object.keys(stateShadow[inp]).map(prop=>{
        return prop!=="color" && stateShadow[inp][prop].slice(-2) !== "px"? `${stateShadow[inp][prop]}px` : stateShadow[inp][prop]
      })
  ...
  const handleOnBlur = (ev,i) => {
    handleDispachUpdateComponent({...input, [i]:{...input[i],[ev.target.name]:ev.target.value}});
    const homeSettingsDiv = document.querySelector('.home-settings');
  ...
  return (
    <div className="shadow-container">
      <div className="shadow-container05">
        <div className="shadow-component-header1">
          ...
          <svg viewBox="0 0 1024 1024" className="shadow-icon06" onClick={addSection}>
            <path d="M213.333 554.667h256v256c0 23.552 19.115 42.667 42.667 42.667s42.667-19.115 42.667-42.667v-256h256c23.552 0 42.667-19.115 42.667-42.667s-19.115-42.667-42.667-42.667h-256v-256c0-23.552-19.115-42.667-42.667-42.667s-42.667 19.115-42.667 42.667v256h-256c-23.552 0-42.667 19.115-42.667 42.667s19.115 42.667 42.667 42.667z"></path>
          </svg>
        </div>
        {
          Object.keys(input).map((inp,i)=><div key={inp} className="shadow-container06">
            <input 
              type="color" 
            ...
            <div className="shadow-container08">
              <input className="shadow-text02" name="ejeX" value={input[i].ejeX} onFocus={(ev)=>handleOnFocus()} onChange={(ev)=>handleInputChange(ev,i)} onKeyDown={(ev)=>handleKeyDown(ev,i)} onWheel={(ev)=>handleScroll(ev,i)} onBlur={(ev)=>handleOnBlur(ev,i)}/>
              <span className="shadow-text03">X</span>
            </div>
            <div className="shadow-container09">
              <input className="shadow-text02" name="ejeY" value={input[i].ejeY} onFocus={(ev)=>handleOnFocus()} onChange={(ev)=>handleInputChange(ev,i)} onKeyDown={(ev)=>handleKeyDown(ev,i)} onWheel={(ev)=>handleScroll(ev,i)} onBlur={(ev)=>handleOnBlur(ev,i)}/>
              <span className="shadow-text05">Y</span>
            </div>
            <div className="shadow-container10">
              <input className="shadow-text02"  name="blur" value={input[i].blur} onFocus={(ev)=>handleOnFocus()} onChange={(ev)=>handleInputChange(ev,i)} onKeyDown={(ev)=>handleKeyDown(ev,i)} onWheel={(ev)=>handleScroll(ev,i)} onBlur={(ev)=>handleOnBlur(ev,i)}/>
              <span className="shadow-text07">Blur</span>
            </div>
            <div className="shadow-container11">
              <input className="shadow-text02"  name="spread" value={input[i].spread} onFocus={(ev)=>handleOnFocus()} onChange={(ev)=>handleInputChange(ev,i)} onKeyDown={(ev)=>handleKeyDown(ev,i)} onWheel={(ev)=>handleScroll(ev,i)} onBlur={(ev)=>handleOnBlur(ev,i)}/>
              <span className="shadow-text09">Spread</span>
            </div>
            <svg viewBox="0 0 1024 1024" className="shadow-icon08" onClick={()=>deleteSection((i))}>
