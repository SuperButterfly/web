...
import { FirstListInputsMedia } from './FirstListInputsMedia.js';
import { SecondListInputsMedia } from './SecondListInputsMedia.js';
import { ThirdListInputsMedia } from './ThirdListInputsMedia.js';
...
const AythenShow = ({ indexList, onClose, inputValue, fixedList, handleChangeInput }) => {
  const [classnameChange, setClassnameChange] = useState("")
  const [stateList, setStateList] = useState(["focus", "hover", "active", "disabled", "checked", "focus-within", "focus-visible"])
  const [stateQuery, setStateQuery] = useState([])
  const [stateListAdd, setStateListAdd] = useState([])
  const [showList, setShowList] = useState({ first: false, second: false, third: false, fourth: false, fifth: false, indexMenu: 0 })
  const [showQuerys, setShowQuerys] = useState({ showQuery1: false, showQuery2: false, showQuery3: false, showQuery4: false, showQuery5: false, showQuery6:false, })
...
*/
  useEffect(() => {
    if (projectSelected.classList) {
      const result = projectSelected.classList?.filter((item) => item.nameClass == inputValue)
      if (result?.length > 0) {
        const newPropertiesList = result[0].properties.states
        const resultingState = Object.entries(newPropertiesList || {}).map(([key, value]) => ({ key, value }));
        if (resultingState) {
  ...
  const handleShowList = () => {
    ...
    setShowList({ first: !showList.first, second: false, third: false, fourth: false, fifth: false })
  }
  const showPropertiesList = () => {
    setShowList({ first: false, second: !showList.second, third: false, fourth: false, fifth: false })
  }
  const showValueList = () => {
    setShowList({ first: false, second: false, third: !showList.third, fourth: false, fifth: false })
  }
  const showFourthList = (index) => {
    //console.log(propertyAddList)
    setShowList({ first: false, second: false, third: false, fourth: !showList.fourth, fifth: false, indexMenu: index })
  }
  const showFifthList = (key, index) => {
    setInputProperty(key);
    //setInputProperty(key)
    setShowList({ first: false, second: false, third: false, fifth: !showList.fifth, fourth: false, indexMenu: index })
  }
  ...
  const handleAddProperties = (index, item, listObject) => {
    ...
    if (item) {
      ...
      if (listObject.key != [indexesList]) {
        ...
        setShowList((prev) => ({ ...prev, fourth: false, fifth: !showList.fifth }))
      }
    }
  }
  ...
  const handleAddValue = (itemList) => {
    if (itemList && inputProperty) {
      const newPropertyAddList = { ...propertyAddList, [inputProperty]: itemList }
              ...
              <button className="pt-btn-transparent-accent" onClick={onClose}>
                <div className="pt-icon-actions">
                  ...
                  >
                    ...
                    <g clip-path="url(#close_svg__clip0)">
        ...
        <div className="panel-content">
          ...
          >
            <div className="section-header-hover">
              <div className="panel-title-container panel-states-container" onClick={handleShowList}>
                <span className="panel-title">States</span>
              <div className="jsx-2997701117">
                <button className="pt-btn-transparent" onClick={handleShowList}>
            ...
            {showList.first && <div className="collapsable-container">
                {stateList?.map((item, index)=>{
                    return(
                     <div className="collapsable-container-list" key={index} onClick={()=>handleAddClick(index)}>
            ...
            {stateListAdd.length == 0 ? null : <div className="section-content">
                ...
                {stateListAdd?.map((item, index)=>{
                  return(
                      <li data-index="0" className="option" key={index}>
                        ...
                        <button className="delete-btn" onClick={()=>handleRemoveItem(item.key)}>
                    ...
                    <div className="input-wrapper">
                      <div className="left-input">
                        <div className="jsx-108338622">
                          ...
                          {showList.second && <div className="collapsable-container-second">
                        ...
                        <div className="jsx-108338622">
                          <div className="pt-input-flexible">
                            <div className="input-addon-wrapper">
                              ...
                              ></input>
                              {showList.third && <div className="collapsable-container-third">
            ...
            <div className="section-content">
              ...
              >
                <div className="sub-panel-section">
                  { breakpoints.map((breakpoint, index) => {
                    ...
                    return (
                      ...
                      <div className="collapsable-header">
                        <div className="jsx-2361534736">&lt;= {mediaQuerys[index]}px</div>
                          <div className="expand-icon-rotate">
                            <div className={showQuerys[`showQuery${index + 1}`] ?  "pt-icon" : "pt-icon pt-icon-rotate"} onClick={()=>setShowQuerys((prev)=>({...prev, [`showQuery${index + 1}`]: !showQuerys[`showQuery${index + 1}`]}))}>
                              <svg
                                width="12"
                                height="9"
                                viewBox="0 0 12 9"
                  ...
                  {showQuerys[`showQuery${index + 1}`] && <FirstListInputsMedia
                    propertyKeys={propertyKeys} 
                    objProperties={objProperties}
