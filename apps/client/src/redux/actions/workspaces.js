...
export const shareWorkspace =
  (id, usermail, collaborator) => async (dispatch) => {
    try {
      const { data } = await axios.patch(
        `/workspace/shareAdd/${id}?collaborator=${collaborator}`,
        { email: usermail }
      ) // add collaborator
      dispatch(updateworkspaceSelected(data.workspace))
    } catch (error) {
...
export const shareUpdate = (id, usermail, collaborator) => async (dispatch) => {
  ...
  try {
    const { data } = await axios.patch(
      `/workspace/shareUpdate/${id}?collaborator=${collaborator}`,
      { email: usermail }
    ) // edit collaborator
    dispatch(updateworkspaceSelected(data.workspace))
  } catch (error) {
...
export const shareRemove = (id, usermail) => async (dispatch) => {
  ...
  try {
    const { data } = await axios.patch(`/workspace/shareRemove/${id}`, {
      email: usermail
    }) // remove collaborator
