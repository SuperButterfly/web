...
const ZIndex = () => {
  ...
  const handleScroll = (ev) => {
    ...
    setInput((prevInput) => {
      const newZIndex =
        parseInt(prevInput.zIndex, 10) - delta;
  ...
  useEffect(()=>{
    if(componentSelected.properties&&componentSelected.properties.style)
      setInput({
        zIndex:componentSelected.properties.style.zIndex?componentSelected.properties.style.zIndex:"100"
      })
  },[id])
  return (
    <div className="-index--index">
      <div className="-index-container">
        ...
        {!isOpen && <svg viewBox="0 0 1024 1024" className="-index-icon"
           onClick={() => setIsOpen(true)}
        >
          <path d="M213.333 554.667h256v256c0 23.552 19.115 42.667 42.667 42.667s42.667-19.115 42.667-42.667v-256h256c23.552 0 42.667-19.115 42.667-42.667s-19.115-42.667-42.667-42.667h-256v-256c0-23.552-19.115-42.667-42.667-42.667s-42.667 19.115-42.667 42.667v256h-256c-23.552 0-42.667 19.115-42.667 42.667s19.115 42.667 42.667 42.667z"></path>  
        </svg>}
        {isOpen && <svg viewBox="0 0 1024 1024" className="-index-icon2"
           onClick={() => setIsOpen(false)}
        >
          <path d="M213.333 554.667h597.333c23.552 0 42.667-19.115 42.667-42.667s-19.115-42.667-42.667-42.667h-597.333c-23.552 0-42.667 19.115-42.667 42.667s19.115 42.667 42.667 42.667z"></path>
        </svg>}
      </div>
      {isOpen && <div className="-index-container1">
        <span className="-index-text1">Z-Indez</span>
        <input type="text" name="zIndex" value={input.zIndex} onFocus={(ev)=>handleOnFocus()} onChange={handleInputChange} onKeyDown={handleKeyDown} onWheel={handleScroll} onBlur={handleOnBlur} className="-index-textinput" />
      </div>}
    </div>
  )
}
