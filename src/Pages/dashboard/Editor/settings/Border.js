...
const Border = () =>{
  ...
  const borderAttributes = ["borderWidth","borderStyle","borderLeftWidth","borderRightWidth","borderBottomWidth","borderTopWidth","borderColor"]
  const borderMedias = ["borderWidth","borderLeftWidth","borderRightWidth","borderBottomWidth","borderTopWidth"]
  ...
  useEffect(()=>{
    if(componentSelected.properties&&componentSelected.properties.style){
      ...
      if(arrPropStyles.length>0)
        setIndependentBorderOpen(arrPropStyles.find(prop=>borderAttributes.includes(prop)&&prop.length>11))
  ...
  return (
    <div className="border-container">
        <div className="border-component-header">
          ...
          {
            !isOpen?<svg viewBox="0 0 1024 1024" className="shadow-icon06" onClick={handleOpen}>
        ...
        <div className="independet-borders-section" style={isOpen?{display:"flex"}:{display:"none"}}>
          <div className="section-borders">
            <span className="border-text">Color</span>
            <div className="independet-borders-column">
              <input type="text" name="borderColor" onChange={handleInputChange} value={input.borderColor} onBlur={handleBlur} className="border-text01" autoComplete="off"/>
            </div>
          </div>
          <div className="section-borders">
            <span className="border-text">Width</span>
            <div className="independet-borders-column">
              <input name="borderWidth" onChange={handleInputChange} value={input.borderWidth} onBlur={handleBlur} type="text" placeholder="0" className="border-text01" autoComplete="off"/>
              <svg version="1.0" fill={independentBorderOpen?"#14A9FF":"#6B6B6B"} onClick={handleOpenIndependentBorder} xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 48 48"><path d="M11 7.5C11 8.8 13.1 9 24 9s13-.2 13-1.5S34.9 6 24 6s-13 .2-13 1.5zM6 24c0 10.9.2 13 1.5 13S9 34.9 9 24s-.2-13-1.5-13S6 13.1 6 24zm33 0c0 10.9.2 13 1.5 13S42 34.9 42 24s-.2-13-1.5-13S39 13.1 39 24zM11 40.5c0 1.3 2.1 1.5 13 1.5s13-.2 13-1.5S34.9 39 24 39s-13 .2-13 1.5z"/></svg>
            </div>
          </div>
            {
              independentBorderOpen&&<div className="section-independent-borders">
                <input type="text" name="borderTopWidth" value={input.borderTopWidth} onChange={handleInputChange} onBlur={handleBlur} placeholder={input.borderWidth} className="independent-border-text" autoComplete="off"/>
                <div className="section-borders01">
                  <input type="text" name="borderLeftWidth" value={input.borderLeftWidth} placeholder={input.borderWidth} onBlur={handleBlur} onChange={handleInputChange} className="independent-border-text" autoComplete="off"/>
                  <input type="text" name="borderRightWidth" value={input.borderRightWidth} placeholder={input.borderWidth} onBlur={handleBlur} onChange={handleInputChange} className="independent-border-text" autoComplete="off"/>
                </div>
                <input type="text" name="borderBottomWidth" value={input.borderBottomWidth} placeholder={input.borderWidth} onBlur={handleBlur} onChange={handleInputChange} className="independent-border-text" autoComplete="off"/>
              </div>
            }
          <div className="section-borders" style={isOpen ? { display: 'flex'} : { display: 'none' }}>
            <span className="border-text" htmlFor="selectUnitLength">Unit of length</span>       
          ...
          <div className="section-borders">
            ...
            <div className="independet-borders-column">
              <input type="text" onChange={handleInputChange} onBlur={handleBlur} value={input.borderStyle} name="borderStyle" className="border-text01"/>
            </div>
          </div>
        </div>
    </div>  
