...
const deletedMultipleComponents = async (req, res, next) => {
  try {
    ...
    await Promise.all(
      req.body.componentsId.map(
        async (id) =>
          await Component.update({ isDeleted: true }, { where: { id } })
      )
    )
...
const cloneComponents = async (copiedComponent) => {
  ...
  if (
    ...
    copiedComponent.children.length
  ) {
...
const copyStylesComponent = async (req, res, next) => {
  try {
    ...
    const componentCopied = await Component.findByPk(idCopied, {
